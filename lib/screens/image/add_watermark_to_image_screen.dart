...
class AddWatermarkToImageScreen extends StatefulWidget {
  final File selectedFile;
  const AddWatermarkToImageScreen({required this.selectedFile, super.key});

  @override
  State<AddWatermarkToImageScreen> createState() =>
      _AddWatermarkToImageScreenState();
}

class _AddWatermarkToImageScreenState extends State<AddWatermarkToImageScreen> {
  File? _watermarkFile;
  ...
  Future<void> _addWatermark() async {
    ...
    final outPath = path.join(
      ...
      'watermarked_image_${DateTime.now().millisecondsSinceEpoch}.jpg',
    );
    ...
    try {
      ...
      developer.log('Image Watermark Result: $result');
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Image saved to: $outPath')));
    } catch (e) {
      developer.log('Watermark Error: $e');
      ScaffoldMessenger.of(
        context,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Watermark to Image')),
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text(_isProcessing ? 'Applying...' : 'Add Watermark'),
            ),
          ],
        ),
      ),
